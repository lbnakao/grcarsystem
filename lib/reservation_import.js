...
function parseFile(name, buf) {
  ...
  if (head.includes('メニュー別内訳') || (head.includes('部門') && head.includes('メニューGP'))) {
    ...
    return { type: 'corecast_menu', facility, year_month: ym, metrics: { '飲食売上': shokuji } };
  }
  ...
  if (head.includes('科目別売上')) {
    let room = 0, shokuji = 0, reji = 0, inSec = false;
    for (const r of rows) {
      const k = norm(r[0]);
      ...
      const amt = n(r[2]);
      if (k === '室料') room += amt;
      else if (/朝食|夕食|ドリンク|飲食/.test(k)) shokuji += amt;
      else if (/入湯税|宿泊税|税$/.test(k)) continue; // 税は除外
      else reji += amt; // 宴会料・その他
    }
    const ym = ymFromText((rows[0] || [])[0]) || ymFromText(name);
    return { type: 'aipass_kamoku', facility, year_month: ym, metrics: { '客室売上': room, '飲食売上': shokuji, 'レジ売上': reji } };
  }
